...
8. [Deep Copy and Shallow Copy](#deep-copy-and-shallow-copy)
9. [Async and Await](#async-and-await)
10. [Memory Management](#memory-management)
11. [Multithreading vs Multiprocessing](#multithreading-vs-multiprocessing)
...
### **8.1 The Core Problem: Why Copying Matters**

Before we get into shallow vs deep, let's understand the *why*. In Python, variables are **names that point to objects in memory**, not boxes that contain values. When you do `b = a`, you don't create a new object — you create a new name pointing to the **same** object.

```python
a = [1, 2, 3]
b = a

print(id(a))  # e.g., 140234567890123
print(id(b))  # 140234567890123  <-- SAME memory address!

b.append(4)
print(a)  # [1, 2, 3, 4]  <-- modifying b also changed a!
print(b)  # [1, 2, 3, 4]
```
...
|------|------|--------------|
| 1 | `a = [1, 2, 3]` | Object `[1, 2, 3]` created at address X. Name `a` → X |
| 2 | `b = a` | No new object. Name `b` → X (same object!) |
| 3 | `b.append(4)` | The object at X is modified in-place → `[1, 2, 3, 4]` |
| 4 | `print(a)` | `a` still points to X, which now holds `[1, 2, 3, 4]` |

This is called **aliasing** — two names for the same object. Often not what you want.

To create an **actual independent copy**, you need shallow copy or deep copy.

---

### **8.2 Shallow Copy**

A **shallow copy** creates a new outer container, but the inner elements are *still shared references* to the original elements.
...
# For dicts: dict(), .copy(), or {**original}
d_orig = {"a": 1, "b": 2}
d_shallow = d_orig.copy()
```

#### **Shallow copy with a flat list (works fine)**

```python
original = [1, 2, 3]
shallow = original.copy()

print(id(original))  # e.g., 1000 (address of outer list)
print(id(shallow))   # e.g., 2000 (DIFFERENT address — new list object!)

shallow.append(4)
print(original)  # [1, 2, 3]         <-- unchanged
print(shallow)   # [1, 2, 3, 4]      <-- only shallow modified
```

**Why this works:** The outer list is a new object, so `shallow.append(4)` only modifies the new list. And because integers (`1`, `2`, `3`) are **immutable**, there's no way to "modify an integer" to cause surprises — you can only replace them.

#### **Shallow copy with a nested list (the trap!)**

```python
original = [[1, 2], [3, 4], [5, 6]]
shallow = original.copy()

print(id(original))       # e.g., 1000 (outer list address)
print(id(shallow))        # e.g., 2000 (different — new outer list)

print(id(original[0]))    # e.g., 3000 (inner list [1, 2])
print(id(shallow[0]))     # e.g., 3000  <-- SAME as original[0]!
```

**Memory picture after shallow copy:**

```
original ──► [ ref_A, ref_B, ref_C ]   (outer list, address 1000)
                │       │       │
                ▼       ▼       ▼
              [1,2]   [3,4]   [5,6]   (inner lists at 3000, 4000, 5000)
                ▲       ▲       ▲
                │       │       │
shallow  ──► [ ref_A, ref_B, ref_C ]   (outer list, address 2000)
```

Both `original` and `shallow` are separate outer lists, but they point to the **same inner lists**.

**What happens when you mutate an inner list:**

```python
shallow[0].append(99)

print(original)  # [[1, 2, 99], [3, 4], [5, 6]]  <-- original affected!
print(shallow)   # [[1, 2, 99], [3, 4], [5, 6]]
```

**Step-by-step trace:**

| Step | Action | State of `original` | State of `shallow` |
|------|--------|---------------------|--------------------|
| 1 | `original = [[1,2],[3,4],[5,6]]` | `[[1,2],[3,4],[5,6]]` | (doesn't exist yet) |
| 2 | `shallow = original.copy()` | `[[1,2],[3,4],[5,6]]` | `[[1,2],[3,4],[5,6]]` (outer is new, inner shared) |
| 3 | `shallow[0].append(99)` | — looks up `shallow[0]` → inner list at 3000 — | — |
| 4 | — mutates inner list at 3000 → `[1,2,99]` | — | — |
| 5 | Final | `[[1,2,99],[3,4],[5,6]]` | `[[1,2,99],[3,4],[5,6]]` |

**Contrast — replacing vs mutating:**

```python
original = [[1, 2], [3, 4], [5, 6]]
shallow = original.copy()

# REPLACING (assignment) — does NOT affect original
shallow[0] = [99, 99]
print(original)  # [[1, 2], [3, 4], [5, 6]]   <-- unchanged
print(shallow)   # [[99, 99], [3, 4], [5, 6]]
```

Why? `shallow[0] = [99, 99]` doesn't mutate the inner list; it makes `shallow`'s slot 0 point to a *new* list `[99, 99]`. The original inner list at address 3000 is untouched, and `original[0]` still points to it.

**Rule of thumb for shallow copy:**
- Changing the *structure* of the outer container (adding/removing/replacing elements) affects only the copy.
- Mutating an inner mutable object (appending to a nested list, modifying a nested dict) affects **both**.

---

### **8.3 Deep Copy**

A **deep copy** recursively copies everything — the outer container *and* all nested containers, all the way down. The result shares nothing mutable with the original.
...
print(id(original))       # e.g., 1000
print(id(deep))           # e.g., 2000 (different outer)

print(id(original[0]))    # e.g., 3000
print(id(deep[0]))        # e.g., 6000 (different inner too!)
```

**Memory picture after deep copy:**

```
original ──► [ ref_A, ref_B, ref_C ]   (outer list, 1000)
                │       │       │
                ▼       ▼       ▼
              [1,2]   [3,4]   [5,6]   (originals at 3000, 4000, 5000)

deep     ──► [ ref_D, ref_E, ref_F ]   (outer list, 2000)
                │       │       │
                ▼       ▼       ▼
              [1,2]   [3,4]   [5,6]   (NEW copies at 6000, 7000, 8000)
```

**Mutation on deep copy is safe:**

```python
deep[0].append(99)

print(original)  # [[1, 2], [3, 4], [5, 6]]           <-- untouched!
print(deep)      # [[1, 2, 99], [3, 4], [5, 6]]
```

**Step-by-step trace:**

| Step | Action | State of `original` | State of `deep` |
|------|--------|---------------------|-----------------|
| 1 | `original = [[1,2],[3,4],[5,6]]` | `[[1,2],[3,4],[5,6]]` | — |
| 2 | `deep = copy.deepcopy(original)` | `[[1,2],[3,4],[5,6]]` | `[[1,2],[3,4],[5,6]]` (everything independent) |
| 3 | `deep[0].append(99)` | — looks up `deep[0]` → its own inner list at 6000 — | — |
| 4 | — mutates inner list at 6000 → `[1,2,99]` | — | — |
| 5 | Final | `[[1,2],[3,4],[5,6]]` | `[[1,2,99],[3,4],[5,6]]` |

---

### **8.4 Visual Side-by-Side Comparison**

Consider: `original = [[1, 2], [3, 4]]`
...
Shallow copy (b = a.copy()):
    a ──► [ref_1, ref_2]             (outer A)
              │      │
              ▼      ▼
            [1,2]  [3,4]              (SHARED inner objects)
              ▲      ▲
              │      │
    b ──► [ref_1, ref_2]             (outer B, different object)
...
Deep copy (b = copy.deepcopy(a)):
    ...
    b ──► [ref_3, ref_4]             (outer B — fully independent)
              │      │
              ▼      ▼
            [1,2]  [3,4]              (inner B's — independent copies)
```

---

### **8.5 Concrete Demonstration — All Three Side by Side**
...
# Three variants
alias    = original              # assignment (same object)
shallow  = copy.copy(original)   # shallow copy
deep     = copy.deepcopy(original)  # deep copy

# Test 1: Mutate an inner list via `shallow`
shallow[0].append(99)

print("After shallow[0].append(99):")
print(f"  original = {original}")  # [[1, 2, 99], [3, 4]]  <-- AFFECTED
print(f"  alias    = {alias}")     # [[1, 2, 99], [3, 4]]  <-- AFFECTED (same object)
print(f"  shallow  = {shallow}")   # [[1, 2, 99], [3, 4]]
print(f"  deep     = {deep}")      # [[1, 2], [3, 4]]      <-- SAFE

# Test 2: Replace an element on `shallow`
shallow[0] = ["new"]

print("\nAfter shallow[0] = ['new']:")
print(f"  original = {original}")  # [[1, 2, 99], [3, 4]]  <-- unchanged by replacement
print(f"  shallow  = {shallow}")   # [['new'], [3, 4]]
```

---

### **8.6 How `copy.deepcopy` Handles Tricky Cases**

#### **Case 1: Immutable objects (ints, strings, tuples of immutables)**

`deepcopy` is smart — it doesn't actually copy immutable objects because there's no reason to. You cannot mutate them, so sharing is safe.

```python
import copy

a = (1, 2, 3)  # tuple of ints — fully immutable
b = copy.deepcopy(a)
...
#### **Case 2: Tuples containing mutable objects**

A tuple is immutable, but if it *contains* a list, the list inside is still mutable. Deep copy handles this correctly:
...
print(a is b)           # False — new tuple created (because inner list needed copying)
print(a[2] is b[2])     # False — inner list was deep-copied
...
`deepcopy` gracefully handles cycles using an internal memo dictionary that tracks already-copied objects:
...
b = copy.deepcopy(a)  # doesn't infinite-loop
print(b)  # [1, 2, [...]]
...
### **8.7 Performance: Deep Copy Is Expensive**

Deep copy walks the entire object graph and constructs new objects for every mutable node. For large nested structures, this is slow and memory-hungry.

```python
import copy
import time

big = [[i] * 100 for i in range(10000)]  # 10,000 lists of 100 ints each

start = time.time()
shallow = copy.copy(big)
print(f"Shallow copy: {time.time() - start:.4f}s")  # e.g., ~0.0001s

start = time.time()
deep = copy.deepcopy(big)
print(f"Deep copy:    {time.time() - start:.4f}s")  # e.g., ~0.05s — ~500x slower
```

**Rule:** Don't reach for `deepcopy` by default. Use it only when you actually need full independence.
...
| Situation | Use |
|-----------|-----|
| Flat container (list of ints, dict of strings, etc.) | Shallow copy — same effect as deep, much faster |
| Nested structure, and you'll mutate inner parts | **Deep copy** |
| Nested structure, but you'll only read inner parts | Shallow copy (cheaper) |
| You want two names for the same object | Simple assignment (`b = a`) |
| Config objects or immutable data | No copy needed — immutability already gives safety |
...
# USE CASE 1: Default config with per-job overrides
base_config = {
    "source": {
        "bucket": "data-lake",
        "prefix": "raw/"
...
# BAD — shallow copy, then mutating nested dict corrupts the base!
job_a_config = base_config.copy()
job_a_config["processing"]["batch_size"] = 5000
print(base_config["processing"]["batch_size"])  # 5000  <-- base corrupted!

# GOOD — deep copy gives true independence
job_b_config = copy.deepcopy(base_config)
job_b_config["processing"]["batch_size"] = 20000
job_b_config["processing"]["retry_policy"]["max_attempts"] = 5
print(base_config["processing"]["batch_size"])              # 5000 (well, corrupted from above, but in clean run: unchanged)
print(base_config["processing"]["retry_policy"]["max_attempts"])  # 3 — safe!

# USE CASE 2: Snapshotting records before mutation
def transform_with_audit(record):
    """Keep original, return transformed — both independent."""
    before = copy.deepcopy(record)  # snapshot for audit log
    
    # Mutate in place
    record["fund_code"] = record["fund_code"].strip().upper()
    record["aum"] = float(record["aum"])
    if "metadata" in record:
        record["metadata"]["processed_at"] = "2025-01-15"
    
    return {"before": before, "after": record}

# USE CASE 3: Row templates in test fixtures
row_template = {
    "fund_code": "FND000",
    "share_class": "A",
    "tags": [],          # mutable!
...
# BAD — all rows share the same `tags` and `metadata` objects
rows_bad = [row_template.copy() for _ in range(3)]
rows_bad[0]["tags"].append("equity")
print(rows_bad[1]["tags"])  # ['equity']  <-- leaked!

# GOOD — deep copy gives each row its own mutable parts
rows_good = [copy.deepcopy(row_template) for _ in range(3)]
rows_good[0]["tags"].append("equity")
print(rows_good[1]["tags"])  # []  <-- isolated

# USE CASE 4: PySpark — DataFrames are immutable, so copy concerns don't apply the same way
# But for Python-side dicts you attach to Spark jobs, deep-copy when forking per partition.
```
...
- `b = a`: two names, one object. Mutations visible through both.
- `b = copy.copy(a)` (shallow): new outer container, shared inner objects. Safe for flat data, risky for nested.
- `b = copy.deepcopy(a)` (deep): fully independent, all the way down. Safest, but slowest.
- Immutable objects (ints, strings, frozen tuples) never need copying — they can't be mutated in place.
- When in doubt with nested mutable data that you'll modify: **deep copy**. The cost of corruption is almost always worse than the cost of copying.

---

## **9. Async and Await** {#async-and-await}

### **9.1 The Motivating Problem**

Imagine you need to fetch data from 10 different APIs. Each takes ~1 second (mostly waiting on the network). How long does the total take?
...
start = time.time()
results = [fetch(f"api/{i}") for i in range(10)]
print(f"Took: {time.time() - start:.1f}s")  # ~10 seconds
```

You waited 10 full seconds — but for most of that time, **your CPU was doing nothing**. It was just twiddling its thumbs waiting for the network. That's the core insight async solves.

**The problem in concrete terms:**
- CPU-bound work → your CPU is actually busy computing
- I/O-bound work (network, disk, database) → your CPU is *waiting* for some external system

For I/O-bound work, we want to say: "While we're waiting on request 1, start request 2. While we're waiting on 2, start 3." That way waits overlap instead of stacking.

**Three ways to overlap I/O in Python:**

| Approach | Cost | Notes |
|----------|------|-------|
| Multiple threads | High — OS threads, context switches, GIL quirks | Works but heavier |
| Multiple processes | Very high — full process overhead | Overkill for I/O |
| **`asyncio`** — single thread, cooperative scheduling | **Cheap** — tens of thousands of tasks on one thread | What this section covers |

---

### **9.2 The Mental Model: Coroutines and the Event Loop**

A **coroutine** is a function that can *pause itself* at specific points, hand control back to a scheduler, and be resumed later. In Python, you define a coroutine using `async def`.

The **event loop** is the scheduler. It juggles many coroutines, running whichever one is ready to make progress.

The magic word is **`await`**: it means "pause this coroutine here, let the event loop run something else, and wake me up when what I'm waiting on is ready."

```python
import asyncio

async def say_hello():        # `async def` → this is a coroutine function
    print("Hello")
    await asyncio.sleep(1)    # pause here for 1 second — yield to event loop
    print("World")

# Calling a coroutine function does NOT run it!
coro = say_hello()
print(coro)  # <coroutine object say_hello at 0x...>  <-- it's a coroutine object

# You run it via the event loop
asyncio.run(say_hello())
```

**What `asyncio.run` does under the hood:**
1. Creates an event loop
2. Schedules `say_hello()` as the first task
3. Runs the loop until that task completes
4. Closes the loop

---

### **9.3 The "Aha" Example: Sequential vs Concurrent**
...
async def fetch(name, delay):
    print(f"  [{time.time()-start:.2f}s] {name} starting")
    await asyncio.sleep(delay)    # simulates network wait
    print(f"  [{time.time()-start:.2f}s] {name} done")
    return f"{name}'s data"

# VERSION A — sequential (awaits one at a time)
...
print(f"Sequential total: {time.time() - start:.2f}s\n")
# Output: ~6.00s total
#   [0.00s] A starting
#   [2.00s] A done
#   [2.00s] B starting
...
# VERSION B — concurrent (all three run together)
...
print(f"Concurrent total: {time.time() - start:.2f}s")
# Output: ~2.00s total
#   [0.00s] A starting
#   [0.00s] B starting
#   [0.00s] C starting
...
**Step-by-step trace of the concurrent version:**

| Time | What the event loop is doing |
|------|------------------------------|
| 0.00s | `gather` schedules A, B, C as tasks |
| 0.00s | Runs A → prints "A starting" → hits `await asyncio.sleep(2)` → A paused, timer set for 2.00s |
| 0.00s | Runs B → prints "B starting" → hits `await asyncio.sleep(2)` → B paused |
| 0.00s | Runs C → prints "C starting" → hits `await asyncio.sleep(2)` → C paused |
| 0.00s – 2.00s | Event loop idle (all three tasks waiting on timers) |
| 2.00s | A's timer fires → A resumes → prints "A done" → returns |
| 2.00s | B's timer fires → B resumes → prints "B done" → returns |
| 2.00s | C's timer fires → C resumes → prints "C done" → returns |
| 2.00s | `gather` has all three results → `concurrent()` returns |

The three 2-second waits **overlapped** instead of stacking → total ~2s instead of 6s.

---

### **9.4 The Essential Syntax**
...
# 2. await ANOTHER coroutine (only valid inside `async def`)
async def caller():
    result = await my_coro()  # pauses caller until my_coro finishes
    return result + 1

# 3. RUN from synchronous code — use asyncio.run
final = asyncio.run(caller())
print(final)  # 43
```

**Three rules you cannot break:**

1. `await` is **only valid inside an `async def`**. Using it at module level or in a regular `def` is a `SyntaxError`.
2. Calling `my_coro()` does **not execute** the coroutine body — it just creates a coroutine object. You must `await` it or schedule it on the loop.
3. `asyncio.run(coro)` is how you bridge from sync code into the async world. Call it **once** at the top of your program — don't call it repeatedly.

---

### **9.5 Running Multiple Coroutines Concurrently**
...
# PATTERN 1: asyncio.gather — run N coroutines, wait for all, collect results
async def main1():
    results = await asyncio.gather(
        task("A", 1),
        task("B", 2),
    ...
    print(results)  # ['A', 'B', 'C'] — order preserved, takes ~3s (longest)

asyncio.run(main1())

# PATTERN 2: asyncio.create_task — schedule a coroutine without awaiting it yet
async def main2():
    t1 = asyncio.create_task(task("A", 1))  # starts running immediately
...
# PATTERN 3: asyncio.gather with error handling — return_exceptions=True
...
async def main3():
    results = await asyncio.gather(
        flaky("A"), flaky("B"), flaky("C"),
        return_exceptions=True,  # exceptions returned as values instead of raising
    )
    for r in results:
...
- `gather(coro1, coro2, coro3)` — the common case. Launch all, wait for all, done.
- `create_task(coro)` — when you want finer control: start a task, do something else, decide later when to await it. Useful for fire-and-forget background work too.

---

### **9.6 `await` Under the Hood — Line-by-Line**

Let's trace what happens during a single `await` call:

```python
async def fetch_user(user_id):
    print("Step 1: about to await")
    data = await some_io_operation(user_id)   # ← THE IMPORTANT LINE
    print("Step 3: got data")
    return data
```

When `await some_io_operation(user_id)` executes:

1. **Step 1** prints "Step 1: about to await".
2. `some_io_operation(user_id)` is called — it returns a coroutine (or awaitable).
3. `await` tells the event loop: *"I'm parking. Wake me up when this awaitable is done. In the meantime, go run other coroutines."*
4. The event loop marks `fetch_user` as suspended and goes to run other ready tasks.
5. Eventually the I/O finishes (data arrives from the network).
6. The event loop schedules `fetch_user` to resume.
7. `await` returns the result of the I/O, binding it to `data`.
8. **Step 3** prints "Step 3: got data".
9. Function returns.

**Key intuition:** `await` is a *negotiation point*. Your coroutine says "I have nothing to do until this is ready"; the event loop says "great, I'll work on other stuff".

---

### **9.7 Async Context Managers and Async For**

Just as `with` has `async with`, and `for` has `async for`, for cases where setup/teardown or iteration themselves involve I/O.

```python
import asyncio

# async with — for resources whose setup/teardown is async
# (e.g., async database connections, aiohttp sessions)
async def example():
    async with open_async_connection() as conn:   # pseudo-code
        data = await conn.query("SELECT ...")
    # conn closed asynchronously on exit

# async for — for async iterators
# (e.g., iterating over rows from a streaming database cursor)
async def stream_rows():
    async for row in db.stream("SELECT * FROM big_table"):   # pseudo-code
        await process(row)
```

You only need these when the library you're using is async-aware. Regular `with open(...)` for local files is still fine inside an `async def`.

---

### **9.8 The Biggest Gotcha: Blocking Calls**

Async only helps if you use **async-aware** I/O libraries. Using a synchronous blocking call inside a coroutine **blocks the entire event loop** — every other task stops making progress.

```python
import asyncio
import time
import requests  # synchronous HTTP library — blocks!

# BAD — time.sleep blocks the event loop
async def bad_sleep():
    time.sleep(2)  # ← blocks everything for 2s!

# GOOD — asyncio.sleep yields to the event loop
async def good_sleep():
    await asyncio.sleep(2)

# BAD — requests.get is synchronous and blocks
async def bad_fetch(url):
    response = requests.get(url)  # ← blocks event loop
    return response.text

# GOOD — use aiohttp or httpx (async HTTP clients)
import aiohttp
...
**Commonly needed async-aware libraries:**

| Blocking (don't use in async code) | Async alternative |
|-------------------------------------|-------------------|
...
| `boto3` | `aioboto3` |
| `open()` for huge files | `aiofiles` |

**Escape hatch — `asyncio.to_thread`:** if you *must* call a blocking function, run it in a thread pool so it doesn't block the loop:

```python
async def use_blocking_lib():
    # Runs blocking code in a separate thread so the event loop keeps running
    result = await asyncio.to_thread(requests.get, "https://example.com")
    return result
```

This is useful when a library has no async version but you don't want to block your other tasks.

---

### **9.9 Timeouts and Cancellation**

Real systems need to give up on hung operations:
...
async def main():
    try:
        ...
        print("Timed out!")  # <-- this path runs

asyncio.run(main())  # prints "Timed out!"
```

Cancellation is cooperative — `wait_for` sends a `CancelledError` into the coroutine, which gets a chance to clean up:
...
### **9.10 Async vs Threading vs Multiprocessing**

All three enable concurrency, but they solve different problems:

| | Async | Threading | Multiprocessing |
|---|-------|-----------|-----------------|
| **Best for** | I/O-bound, many tasks | I/O-bound, few tasks or blocking libraries | CPU-bound |
| **Overhead per task** | ~few KB | ~few MB (OS thread) | ~MB+ (whole process) |
| **Max concurrent tasks** | 10,000s | ~100s | ~CPU cores |
| **GIL limitation?** | N/A (single thread) | Yes — no parallel CPU | No — true parallelism |
| **Shared memory?** | Yes (single thread) | Yes (same process) | No (need IPC) |
| **Complexity** | Need async libraries everywhere | Locks, races | Serialization, IPC |
| **Code style** | `async`/`await` | `Thread`, `ThreadPoolExecutor` | `Process`, `Pool` |

**Rule of thumb:**
- Thousands of concurrent I/O operations (API calls, web scraping, chat servers) → **async**
- Moderate I/O parallelism with stubbornly-sync libraries → **threading**
- Heavy CPU work (number-crunching, encoding, parsing) → **multiprocessing**

See the following section on Multithreading vs Multiprocessing for more detail on the latter two.
...
# USE CASE 1: Concurrently fetch many API endpoints (data enrichment)
async def fetch_fund_metadata(session, fund_code):
    url = f"https://api.example.com/funds/{fund_code}"
    async with session.get(url) as response:
        return await response.json()

async def enrich_funds(fund_codes):
    """Fetch metadata for hundreds of funds concurrently."""
    async with aiohttp.ClientSession() as session:
        tasks = [fetch_fund_metadata(session, code) for code in fund_codes]
        results = await asyncio.gather(*tasks, return_exceptions=True)
    
    enriched = {}
    for code, result in zip(fund_codes, results):
        if isinstance(result, Exception):
            print(f"Failed for {code}: {result}")
        else:
            enriched[code] = result
    return enriched

# Run it
fund_codes = ["FND001", "FND002", ..., "FND500"]  # 500 funds
metadata = asyncio.run(enrich_funds(fund_codes))
# Takes seconds instead of minutes

# USE CASE 2: Concurrent S3 uploads with aioboto3
import aioboto3

async def upload_file(s3_client, bucket, key, local_path):
    await s3_client.upload_file(local_path, bucket, key)
    return key

async def upload_batch(bucket, files):
    """files: list of (key, local_path) tuples."""
    session = aioboto3.Session()
    async with session.client("s3") as s3:
        tasks = [upload_file(s3, bucket, key, path) for key, path in files]
        return await asyncio.gather(*tasks)

# USE CASE 3: Rate-limited concurrent requests via a semaphore
async def rate_limited_fetch(session, semaphore, url):
...
async def fetch_with_rate_limit(urls, max_concurrent=10):
    ...
    async with aiohttp.ClientSession() as session:
        tasks = [rate_limited_fetch(session, semaphore, url) for url in urls]
        return await asyncio.gather(*tasks)

# Fetch 1000 URLs, but only 10 at a time — respectful of the API
urls = [f"https://api.example.com/item/{i}" for i in range(1000)]
results = asyncio.run(fetch_with_rate_limit(urls, max_concurrent=10))

# USE CASE 4: Concurrent database writes (async-aware driver)
import asyncpg

async def insert_record(pool, record):
    async with pool.acquire() as conn:
        await conn.execute(
            "INSERT INTO facts (fund_code, aum, date) VALUES ($1, $2, $3)",
            record["fund_code"], record["aum"], record["date"]
        )

async def bulk_insert(records, db_url):
    pool = await asyncpg.create_pool(db_url, min_size=5, max_size=20)
    try:
        await asyncio.gather(*[insert_record(pool, r) for r in records])
    finally:
        await pool.close()

# USE CASE 5: Parallel Redshift Data API calls (statement status polling)
import aioboto3

async def wait_for_statement(redshift_client, statement_id):
    """Poll a Redshift Data API statement until it completes."""
    while True:
        response = await redshift_client.describe_statement(Id=statement_id)
        status = response["Status"]
        if status in ("FINISHED", "FAILED", "ABORTED"):
            return response
        await asyncio.sleep(1)  # yields to event loop — doesn't block

async def run_parallel_queries(queries, workgroup, db):
    session = aioboto3.Session()
    async with session.client("redshift-data") as client:
        # Kick off all queries
        statement_ids = []
        for q in queries:
            r = await client.execute_statement(
                WorkgroupName=workgroup, Database=db, Sql=q
            )
            statement_ids.append(r["Id"])
        
        # Wait for all in parallel
        results = await asyncio.gather(
            *[wait_for_statement(client, sid) for sid in statement_ids]
        )
        return results
```
...
### **9.12 Common Mistakes and How to Spot Them**

```python
# MISTAKE 1: Forgetting to await — coroutine just never runs
async def fetch_data():
    return "data"

async def main():
    result = fetch_data()          # WRONG: `result` is a coroutine object, not "data"
    result = await fetch_data()    # RIGHT

# Python will usually give you a RuntimeWarning:
#   "coroutine 'fetch_data' was never awaited"

# MISTAKE 2: Calling asyncio.run() inside an already-running loop
async def outer():
    asyncio.run(inner())   # RuntimeError: cannot be called from a running event loop
    await inner()          # CORRECT

# MISTAKE 3: Using blocking I/O in async code (silently kills concurrency)
async def bad():
    import requests
    r = requests.get("https://example.com")  # blocks loop!
...
# MISTAKE 4: Creating tasks and letting them garbage-collect
async def main():
    asyncio.create_task(background_job())  # task may be GC'd before running!
    # Always keep a reference:
    task = asyncio.create_task(background_job())
    # ...and await it eventually, or add to a set you hold onto

# MISTAKE 5: Expecting async to speed up CPU-bound code
async def cpu_heavy():
    return sum(i*i for i in range(10**8))  # single-threaded, async doesn't help

async def main():
    # These run sequentially in effect — no parallelism for CPU work
    await asyncio.gather(cpu_heavy(), cpu_heavy(), cpu_heavy())
# For CPU work: use multiprocessing, not async.
```
...
- `await` pauses the current coroutine and lets the event loop run other work — only meaningful for I/O-bound operations using async-aware libraries.
- `asyncio.run(coro)` is the one bridge from sync code into the async world; call it once at the top.
- `asyncio.gather(*coros)` runs coroutines concurrently and collects results.
- The event loop is single-threaded — a blocking call (`time.sleep`, `requests.get`) freezes everything. Use async-aware libraries (`asyncio.sleep`, `aiohttp`, `asyncpg`, `aioboto3`) or wrap blocking calls with `asyncio.to_thread`.
- Async is for **I/O-bound concurrency with many concurrent operations**. For CPU-bound work, use multiprocessing instead.

---

## **10. Memory Management** {#memory-management}

### **10.1 How Python Manages Memory**
...
### **10.3 Memory-Efficient Practices**
...
### **10.5 ETL Memory Management Examples**
...
### **11.4 Comparison: Threading vs Multiprocessing**
...
### **Copy Semantics**
- **Assignment (`b = a`)**: two names, one object
- **Shallow copy**: new outer container, shared inner objects — safe for flat data only
- **Deep copy**: fully independent all the way down — use when nested mutation matters

### **Concurrency**
- **Async/await**: best for I/O-bound with many concurrent operations (single thread, event loop)
- **Threading**: best for I/O-bound with fewer tasks or when async libraries aren't available
- **Multiprocessing**: best for CPU-bound tasks (bypasses the GIL)
